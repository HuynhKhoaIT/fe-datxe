...
const fetchCars = async (searchParams: any, page: number): Promise<any> => {
    const response = await fetch(`/api/admin/car?${searchParams}&page=${page}`);
    if (!response.ok) {
...
const fetchCarsDlbd = async (searchParams: any, page: number): Promise<any> => {
    const response = await fetch(`/api/admin/car/dlbd?${searchParams}&page=${page}`);
    if (!response.ok) {
        throw new ResponseError('Failed to fetch cars', response);
    }
    return await response.json();
...
export const useCars = (): useCars => {
    ...
    }, [cars, searchParams, isPlaceholderData, page, queryClient, isPlaceholderDataDlbd]);
